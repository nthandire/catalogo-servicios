			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="desProveedor" title="${message(code: 'proveedor.desProveedor.label', default: 'Des Proveedor')}" />
					
						<g:sortableColumn property="representante" title="${message(code: 'proveedor.representante.label', default: 'Representante')}" />
				...
				<g:each in="${proveedorInstanceList}" status="i" var="proveedorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${proveedorInstance.id}">${fieldValue(bean: proveedorInstance, field: "desProveedor")}</g:link></td>
					
						<td>${fieldValue(bean: proveedorInstance, field: "representante")}</td>
